...
using namespace std;
using namespace cv;
const int imageWidth = 1280;                             //图片尺寸  
...
0  0  1
*/
Mat cameraMatrixL = (Mat_<double>(3, 3) << 1565.9, -18.4913, 745.3755, 0, 1556.0, 246.9201, 0, 0, 1.);
//获得的畸变参数
...
Mat distCoeffL = (Mat_<double>(5, 1) << 0.1038, 0.6804, -0.0208, 0.0297, 0);
...
0  0  1
*/
Mat cameraMatrixR = (Mat_<double>(3, 3) << 1555.4, -18.9175, 694.4408, 0, 1550.0, 228.7629, 0, 0, 1.);
...
Mat distCoeffR = (Mat_<double>(5, 1) << 0.1052, 0.2831, -0.0365, 0.0124, 0);
...
//[-1.210187345641146e+02,0.519235426836325,-0.425535566316217]
															 //rec旋转向量，对应matlab om参数  我
//Mat rec = (Mat_<double>(3, 1) << -0.00306, -0.03207, 0.00206);//rec旋转向量，对应matlab om参数  我 
Mat rec = (Mat_<double>(3, 3) << 0.9984, -0.0483, -0.0297,
	0.0427, 0.9982, -0.0358,
	0.0314, 0.0354, 0.9989);                //rec旋转向量，对应matlab om参数  我  
...
int main()
{
	...
	*/
	...
	imgl = imread("D:/image/left/left_2.jpg", IMREAD_COLOR);
	imgr = imread("D:/image/right/right_2.jpg", IMREAD_COLOR);
	imshow("imgl", imgl);
	//rgbImageL = imgl(m_l_select);
	cvtColor(imgl, grayImageL, COLOR_BGR2GRAY);

	//m_r_select = Rect(640, 0, 640, 480);
	//rgbImageR = imgr(m_r_select);
